...
echo "8:3 1048576" > $CGROUP_CONTAINER_PATH1/blkio.throttle.read_bps_device
echo "8:3 1048576" > $CGROUP_CONTAINER_PATH1/blkio.throttle.write_bps_device
...
# To get the device major and minor id from /dev for the device that /tmp/test1 is on.
echo "8:3 1048576" > $CGROUP_CONTAINER_PATH2/blkio.throttle.read_bps_device
echo "8:3 1048576" > $CGROUP_CONTAINER_PATH2/blkio.throttle.write_bps_device

docker exec fio_test1 fio -direct=1 -rw=write -ioengine=libaio -bs=4k -size=10MB -numjobs=1  -name=/tmp/fio_test1.log &
docker exec fio_test2 fio -direct=1 -rw=write -ioengine=libaio -bs=4k -size=10MB -numjobs=1  -name=/tmp/fio_test2.log &
